...
export function SettingsPanel({ isOpen, onClose }: SettingsPanelProps) {
    ...
    try {
      ...
      if (copilotResult) {
        ...
        if (copilotResult.found && copilotResult.path) {
          await checkCopilotShellProfile();
        }
      }
    } catch (error) {
  ...
  const checkCopilotShellProfile = async () => {
    setCheckingProfile(true);
    try {
      ...
      setIsCopilotInProfile(false);
    } finally {
      setCheckingProfile(false);
    }
  };

  const addCopilotToProfile = async () => {
    try {
      // For standalone copilot, it should already be in PATH
      // For gh extension, add gh to PATH if needed
      const line = 'export PATH="$PATH:/usr/local/bin"';
      await invoke("add_to_shell_profile", { line });
      toast.success("Added to shell profile. Please restart your terminal.");
      setIsCopilotInProfile(true);
    } catch (error) {
      if (error === "Already configured") {
        toast.info("Already configured in shell profile");
        setIsCopilotInProfile(true);
      } else {
        toast.error(`Failed to add to shell profile: ${error}`);
      }
    }
  };
          ...
          <div className="max-w-5xl mx-auto p-8">
            ...
            {activeSection === "ai-copilot" && (
              <CopilotSettings
                ...
                onDetect={detectCopilotCLI}
                shellProfilePath={shellProfilePath}
                isCopilotInProfile={isCopilotInProfile}
                checkingProfile={checkingProfile}
                onAddToProfile={addCopilotToProfile}
              />
            )}
